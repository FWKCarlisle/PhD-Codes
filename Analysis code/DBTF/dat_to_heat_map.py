...
for i, file in enumerate(file_names):
    if file[-1] == "t":
        ...
        z_data, index_list = remove_small_values(z_data, z_data.max() * 0.01)
        print(len(index_list)) # 50% of max value, aritrary threshold, can be changed but this is just to prove the concept
        
        current_data = np.delete(current_data, index_list)
        rotated_x = np.delete(rotated_x, index_list)
        rotated_y = np.delete(rotated_y, index_list)

        create_map(rotated_x, rotated_y, current_data, label="Current (A)", scale=1)
