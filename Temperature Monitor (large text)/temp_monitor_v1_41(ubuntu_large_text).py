...
config_path = pathlib.Path(__file__).parent.absolute() / 'temp_monitor_parameter.ini'
...
TEMP_REGISTER = 4096
#Set up instrument
instrument = minimalmodbus.Instrument("COM3",1,mode=minimalmodbus.MODE_ASCII)
...
class HelloWindow(QMainWindow):
    def __init__(self):
        ...
        title = QLabel("Temperature location", self) 
    ...
    def get_pressure_from_device(self):
        ...
        #Note sometimes fails... use for with a try!
        try:
        ...
        self.all_pressure_values = [temperature/10]
    ...
    def onTimeout(self):
        ...
        if num_of_input >= 1:
            ...
            self.x = self.x + 1
            self.pressure1.setText(input1+' = '+ "%1.2f" %self.all_pressure_values[0] +'C')
